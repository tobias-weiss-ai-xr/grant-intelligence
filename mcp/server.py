...
def ingest(programme: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    new IDs are appended. Changes are saved to catalog.json.

    Args:
        programme: List of program dictionaries with required 'id' field.

    Returns:
        Dictionary with status, counts (neu, aktualisiert, gesamt).
    """
    added, updated = 0, 0
...
def match_best(
    ...
    """
    return [r.__dict__ for r in match_profile(PROGRAMME, felder, karriere, rolle=rolle, top=top)]
...
def naechste_fristen(
    ...
    """
    return [r.__dict__ for r in next_deadline(PROGRAMME, felder, karriere, top=top)]
...
def notify(
    ...
    for r in results:
        ...
        elif r.tage_bis_frist is not None and r.tage_bis_frist <= tage:
            out.append(r.__dict__)
    return out
...
def brief(
    ...
    return {
        "top_matches": [r.__dict__ for r in match_profile(PROGRAMME, felder, karriere, rolle=rolle, top=top)],
        "naechste_frist": fristen[0].__dict__ if fristen else None,
