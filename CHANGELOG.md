...
- **Deduplizierung:** `arc-discovery` und `cihr-irsc` entfernt (semantische Duplikate der bestehenden `arc-international`/`cihr-international`); eine Entität pro Förderer. 110 → 108 Programme.
- **Dokumentations-Frischekurs:** README/CHANGELOG/Konzept/MVP-Demo/Promo/export auf 108 Programme, 28 Quellen, 566 Tests, 99 % Coverage aktualisiert; `docs/brief.md` regeneriert (108 Programme, Stand 2026-09-01).
- **Transparente Match-Scores (`punkte`):** Strukturierte Aufschlüsselung der Punktzahl auf jedem `MatchResult` (`Thema 0–3`, `Karriere 0–1`; Summe == Score). Über MCP-API (`match_best`/`naechste_fristen`) und Wochen-Brief exponiert; Brief zeigt echte Maxima + Komponenten (`3/4 (Thema 2/3 · Karriere 1/1)`) statt irreführendem `/5`. Score-Algorithmus unverändert. 10 neue Tests.
- **`parsers.py` aus `fetchers.py` extrahiert:** reine Parsing-Logik (`slug_id`, `parse_bmbf_rss`) ohne Netzwerk/Zeitbezug – offline deterministisch testbar; HTTP bleibt in `fetchers.py` (Unix: eine Sache). `_slug_id` bleibt als Alias. 12 neue Tests.
