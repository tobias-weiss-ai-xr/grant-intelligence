...
- [x] **Fail-early + Don't-hammer:** `run()` bricht beim ersten `BROKEN` ab (pending Futures gecancelt, Stopp des Hammerns); `broken`-Flag wird aber unabhängig von `fail_early` gesetzt. Politeness: Per-Host-Throttle (`per_host_delay`, Configs 0.5s), `workers` default 4, 429->UNCERTAIN mit `Retry-After`-Backoff. In allen Repos + Tests propagiert; fail-early deterministisch getestet (`--no-fail-early` sammelt alle).
- [x] **Tool-Bug gefixt:** `broken`-Status wurde nur bei `fail_early=True` gesetzt -> mit `--no-fail-early`/`--no-fail` wurden tote Links nicht als BROKEN gezaehlt (falscher Exit-Code). Behoben; Test-Config `fail_early:False` fuer Counting-Tests.
